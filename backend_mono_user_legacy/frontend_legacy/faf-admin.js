...
export class AdminAPI {
  ...
  /**
   * Vérifie si l'utilisateur est authentifié avec un JWT valide
   * Redirige vers /auth/login.html si pas de token ou token invalide
   * @returns {Promise<Object|null>} Les données admin ou null si non authentifié
   */
  ...
  /**
   * Déconnecte l'utilisateur (supprime le JWT et redirige)
   */
  static logout() {
    localStorage.removeItem('faf_token');
    localStorage.removeItem('faf_username');
  ...
  /**
   * Récupère le JWT token depuis localStorage
   * @returns {string|null}
   */
  static getJWT() {
    return localStorage.getItem('faf_token');
  }
  ...
  /**
   * Effectue une requête API avec gestion automatique d'erreurs, JWT et CSRF
   */
  static async request(url, options = {}, errorMessage = 'Erreur de communication') {
    ...
    try {
      ...
      // Ajouter le JWT token pour toutes les requêtes admin
